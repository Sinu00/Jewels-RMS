...
export function OrnamentCard({ ornament, onClick, actionSlot, selected, className }: OrnamentCardProps) {
  ...
  const inner = (
    <div
      className={cn(
        'rounded-2xl bg-card border border-border overflow-hidden cursor-pointer card-interactive',
    ...
    >
      ...
      <div className="aspect-square bg-surface relative overflow-hidden">
        ...
        {/* Price badge overlay */}
        <div className="absolute top-2 right-2 bg-ink/80 text-white text-[10px] font-medium px-2 py-0.5 rounded-full backdrop-blur-sm">
          {new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 }).format(ornament.baseRatePerDay)}/d
        </div>
      </div>
