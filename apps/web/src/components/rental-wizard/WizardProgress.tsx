          ...
          return (
            <div key={i} className="flex items-center">
              <div className="flex flex-col items-center">
                ...
                <span className={cn('mt-1 text-xs hidden sm:block', active ? 'text-ink font-medium' : done ? 'text-muted' : 'text-muted/40')}>
                  {label}
                </span>
              </div>
              {i < STEPS.length - 1 && (
                <div className={cn('h-0.5 w-10 sm:w-16 mx-1.5 sm:mx-2 mb-4 rounded-full transition-colors', done ? 'bg-ink' : 'bg-border')} />
              )}
            </div>
          )
        })}
